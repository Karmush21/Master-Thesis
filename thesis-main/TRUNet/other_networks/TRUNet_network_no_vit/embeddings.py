...
from other_networks.TRUNet_network_no_vit.resnetv2_3d import ResNetV23d
from other_networks.TRUNet_network_no_vit.encode_3d import EncoderCup3d
...
class Embeddings3d(nn.Module):
    ...
    def forward(self, x):
        #Only do the resnet part. hybrid is always true for some reason, don't ask me why they do it like this
        if self.hybrid:
            x, features = self.hybrid_model(x)
        else:
            x, features = self.convolutions(x)

        # All below here is related to the transformer
        ...
        return x, features #1024, 14,14,14
